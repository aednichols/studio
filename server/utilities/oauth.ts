...
import {sendClassCodeAddedEmail, sendWelcomeEmail} from './emails';
...
export async function oAuthCallback(req: express.Request) {
  ...
  if (user && pending) {
    ...
    if (classroom && !classroom.students.includes(user.id)) {
      classroom.students.push(user.id);
      sendClassCodeAddedEmail(user, classroom);  // async
    }
